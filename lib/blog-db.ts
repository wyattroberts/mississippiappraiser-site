...
function databaseConnectionString() {
  ...
  try {
    ...
    // explicit `ssl` object below. DigitalOcean's managed database URL includes
    // sslmode=require, while its certificate chain is not trusted by Node's
    // default CA bundle. Remove URL-level SSL options so the explicit TLS policy
...
export async function getPublishedPosts(limit?: number) {
  ...
  if (!databaseUrl()) return bundledPosts();
  ...
  try {
    ...
    console.error("Blog database unavailable; serving bundled published posts", error);
    return bundledPosts();
  }
}

export async function findPublishedPost(year: string, month: string, slug: string) {
  const bundledPost = () => legacyAllPosts.find(
    (post) => post.status === "published" && post.slug === slug && post.date.slice(0, 7) === `${year}-${month}`,
  );
  if (!databaseUrl()) return bundledPost();
  ...
  try {
    ...
    console.error("Blog database unavailable; serving bundled published post", error);
    return bundledPost();
  }
}
